...
def _apply_config():
    ...
    MAX_BATHROOMS = cfg.get("max_bathrooms", MAX_BATHROOMS)
    MIN_PARKING = cfg.get("min_parking", MIN_PARKING)
    MAX_PARKING = cfg.get("max_parking", MAX_PARKING)
...
# --- Авто-переключение режима по дате (расход кредитов ScraperAPI) ---
# До конца триала (5000 кредитов) качаем по максимуму; после — экономный режим
# под бесплатный ~1000 кредитов/мес. Бот переключается САМ, без нашего участия.
SCRAPERAPI_TRIAL_END = datetime.date(2026, 7, 28)
if datetime.date.today() < SCRAPERAPI_TRIAL_END:
    PAGES_PER_SEARCH = 5   # триал: глубокий охват, большой запас вариантов
    MAX_PER_RUN = 12
else:
